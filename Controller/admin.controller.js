                        ...
                        adminModel.findOne({ 'email': email }, (err, thisAdmin) => {
                            if (err) {
                                ...
                                var mailMessage = {
                                    ...
                                    <p >With Adeyosola varieties, you are to manage, protect and secure the site from unrelevant effect</p>
                                    <b>This is the password for your account ${password}, <i style="color: red;">IT IS REQUIRED WHEN TO LOGIN</i></b>
                                    <p>Click <a href='https://adeyosolavarieties.netlify.app/staff_login' style='text-decoration: none; color: #FF5722;'>link</a> to login
                                    Thank you!`
                                }
                                transporter.sendMail(mailMessage, (err, result) => {
                                    if (err) {
                                        console.log(`Connection error`);
                                        res.send({message: `Connection error, please check you connection`})
                                    }
                                    else {
                                        res.send({ message: `Registration successfull, Please login to the Gmail account ${email} for your private admin key.`, status: true })
                                    }
        ...
        if (err) {
            ...
            form.save((err) => {
                if (err) {
                    ...
                    res.send({productDetail, message: `Product uploaded successfully`, status: true, })
                }
            })
        }
    })
...
const editProduct = (req, res)=>{
    ...
    productModel.findOneAndUpdate({'_id': Id}, { $set: {'title': req.body.editTitle, 'price': req.body.editPrice, 'rating': req.body.editRate} }, (err, result)=>{
        if(err){
            console.log(err);
            res.send({message: `Error occurred, please check your connection!`, status: false})
        }else{
...
module.exports = { signup, signin, authorizeUser, customer, products, deleteStaff, saveProfile, profilePhoto, deleteProduct, deleteAccount, staffSignin, staffSignup, editProduct }